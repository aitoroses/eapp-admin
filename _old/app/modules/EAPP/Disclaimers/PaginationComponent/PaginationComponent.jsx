...
var PaginationFactory = function PaginationComponent(injector, util) {
	...
	var PaginationComponent = React.createClass({
	    ...
	    getInitialState(){
	      return {
	        page: 1,
	        maximumPages: this.disclaimers().store.getMaxPages(),
	        dataLength: null,
	        maxPageArr: null,
	        maxPage: null,
	        showPaginator: true
	    ...
	    onDisclaimersLengthFetch(){
	    	console.log("event count fetched disclaimers");
	    	var arr = this.createButtons(this.disclaimers().store.getDisclaimersCount(),this.disclaimers().store.getTotalRows());
	    	this.setState({
	    		dataLength: this.disclaimers().store.getDisclaimersCount(),
	    		maxPageArr: arr
	    	});
	    },
	    ...
	    setPage(p){
	    	...
	    	if(p>0 && p<this.state.maxPage+1){
	    		this.disclaimers().actions.setPage(p);
	    		this.disclaimers().actions.queryDisclaimers();
	    		this.onDisclaimersLengthFetch();
	    		this.setState({
	    			page: p
	    		});
	    	}
