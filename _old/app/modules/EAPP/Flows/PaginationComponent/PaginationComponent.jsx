...
var PaginationFactory = function PaginationComponent(injector, util) {
	...
	var PaginationComponent = React.createClass({
	    ...
	    getInitialState(){
	      return {
	        page: 1,
	        maximumPages: this.countries().store.getMaxPages(),
	        dataLength: null,
	        maxPageArr: null,
	        maxPage: null,
	        showPaginator: true
	    ...
	    onCountriesLengthFetch(){
	    	console.log("event count fetched countries");
	    	var arr = this.createButtons(this.countries().store.getCountriesCount(),this.countries().store.getTotalRows());
	    	this.setState({
	    		dataLength: this.countries().store.getCountriesCount(),
	    		maxPageArr: arr
	    	});
	    },
	    ...
	    setPage(p){
	    	...
	    	if(p>0 && p<this.state.maxPage+1){
	    		this.countries().actions.setPage(p);
	    		this.countries().actions.queryCountries();
	    		this.onCountriesLengthFetch();
	    		this.setState({
	    			page: p
	    		});
	    	}
