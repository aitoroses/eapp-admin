...
class PaginationComponent extends React.Component {
    ...
    componentWillReceiveProps(nextProps) {
        if(store.getMaxPageArr() == null && store.getLoadPagination()){
            actions.queryItems();
            this.onItemsLengthFetch(nextProps);
            actions.setLoadPagination(false);
        }
    }

    onItemsLengthFetch(nextProps){
        console.log("event count fetched items - pagination");
        var arr = this.createButtons(nextProps.data.itemsLength, store.getTotalRows());
        actions.setDataLength(nextProps.data.itemsLength);
