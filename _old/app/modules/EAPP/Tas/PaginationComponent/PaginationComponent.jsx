...
var PaginationFactory = function PaginationComponent(injector, util) {
	...
	var PaginationComponent = React.createClass({
	    ...
	    getInitialState(){
	      return {
	        page: 1,
	        maximumPages: this.tas().store.getMaxPages(),
	        dataLength: null,
	        maxPageArr: null,
	        maxPage: null,
	        showPaginator: true
	    ...
	    onTasLengthFetch(){
	    	console.log("event count fetched tas");
	    	var arr = this.createButtons(this.tas().store.getTasCount(),this.tas().store.getTotalRows());
	    	this.setState({
	    		dataLength: this.tas().store.getTasCount(),
	    		maxPageArr: arr
	    	});
	    },
	    ...
	    setPage(p){
	    	...
	    	if(p>0 && p<this.state.maxPage+1){
	    		this.tas().actions.setPage(p);
	    		this.tas().actions.queryTas();
	    		this.onTasLengthFetch();
	    		this.setState({
	    			page: p
	    		});
	    	}
