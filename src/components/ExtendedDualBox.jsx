		...
		var selectBox = this.props.right.map(function(option, index){
			...
			if(option.category==null && categories.length>0){
				var optionNodes = categories.map(function(opt){
					return <option selected={opt["name"]=="GENERAL"} key={opt["name"]} value={opt["name"]}>{opt["name"]}</option>
				})
			}else if(categories.length>0){
				var optionNodes = categories.map(function(opt){
					return <option selected={option.category==opt["name"]} key={opt["name"]} value={opt["name"]}>{opt["name"]}</option>
				})
			}
