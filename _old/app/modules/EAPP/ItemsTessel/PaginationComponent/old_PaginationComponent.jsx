...
var PaginationFactory = function PaginationComponent(injector, util) {
	...
	var PaginationComponent = React.createClass({
		...
		/**
	     * Get the initial state fororacle
		  the component
	     */
	    getInitialState(){
	      return {
	        page: 1,
	        maximumPages: this.items().store.getMaxPages(),
	        dataLength: null,
	        maxPageArr: null,
	        maxPage: null,
	        showPaginator: true
	    ...
	    onItemsLengthFetch(){
	    	console.log("event count fetched items");
	    	var arr = this.createButtons(this.items().store.getItemsCount(),this.items().store.getTotalRows());
	    	this.setState({
	    		dataLength: this.items().store.getItemsCount(),
	    		maxPageArr: arr
	    	});
	    },
	    ...
	    setPage(p){
	    	...
	    	if(p>0 && p<this.state.maxPage+1){
	    		this.items().actions.setPage(p);
	    		this.items().actions.queryItems();
	    		this.onItemsLengthFetch();
	    		this.setState({
	    			page: p
	    		});
	    	}
